...
--
-- Host: 127.0.0.1
-- Generation Time: Jan 29, 2023 at 08:13 AM
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `profitpenjualan` ()   BEGIN
	CREATE VIEW profitPenjualan AS
	SELECT SUM(HitungProfit(hargatiket.HargaTiket))
FROM hargatiket;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `PropitBulan` ()   BEGIN
CREATE VIEW ProfitBulan AS
SELECT month(pemesananpergi.Tanggal_Keberangkatan) AS Bulan,
SUM(HitungProfit(hargatiket.HargaTiket)) AS ProfitBulan
FROM pemesananpergi, hargatiket, pembayaran
WHERE pemesananpergi.Nomor_Transaksi_Pergi = pembayaran.Nomor_Pergi AND pembayaran.hargaId = hargatiket.Idharga
GROUP BY month(pemesananpergi.Tanggal_Keberangkatan);
...
--
-- Stand-in structure for view `crowded_schedule2`
-- (See below for the actual view)
--
CREATE TABLE `crowded_schedule2` (
`Bulan` int(2)
...
('CI001', 'imgc1', '123456789012', 'Aurelius', 'Male', '2022-10-02', 82229164338, 'aurelius@gmail.com', '00000001', 'Jl. Wonerejo No.29 B'),
('CI002', 'imgc2', '123456789013', 'Tita', 'Female', '2022-10-03', 82145287612, 'tita@gmail.com', '00000002', 'Jl.Karang sari V No.24'),
('CI003', 'imgc3', '123456789014', 'Kezia', 'Female', '2022-10-04', 81216221733, 'kezia@gmail.com', '00000003', 'Jl. Jagir No.238'),
('CI004', 'imgc4', '123456789015', 'Angel', 'Female', '2022-10-05', 81365889012, 'angel@gmail.com', '00000004', 'Jl. Araya No. B37'),
('CI005', 'imgc5', '123456789016', 'Rafael', 'Male', '2022-10-06', 82156281162, 'rafael@gmail.com', '00000005', 'Jl. Melati Indah No.15'),
('CI006', 'imgc6', '123456789017', 'Pirelli', 'Female', '2022-10-07', 81245637288, 'pirelli@gmail.com', '00000006', 'Jl. Belimbing No.28'),
('CI007', 'imgc7', '123456789018', 'Fangeline', 'Female', '2022-10-08', 82416772201, 'fangeline@gmail.com', '00000007', 'Jl. Mekar Raya No.10'),
('CI008', 'imgc8', '123456789019', 'William', 'Male', '2022-10-09', 81266242412, 'william@gmail.com', '00000008', 'Jl. Kawi No.6'),
('CI009', 'imgc9', '123456789010', 'Andrew', 'Male', '2022-10-10', 81077332424, 'andrew@gmail.com', '00000009', 'Jl. Ahmad Yani No.4'),
('CI010', 'imgc10', '123456789011', 'Josesdio Dio', 'Male', '2022-10-11', 82145664422, 'josesdio@gmail.com', '00000010', 'Jl. Wonokromo No.8');
...
--
-- Stand-in structure for view `profitbulan`
-- (See below for the actual view)
--
CREATE TABLE `profitbulan` (
`Bulan` int(2)
,`ProfitBulan` decimal(32,0)
);
...
--
-- Structure for view `crowded_schedule2`
--
DROP TABLE IF EXISTS `crowded_schedule2`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `crowded_schedule2`  AS SELECT month(`pemesananpergi`.`Tanggal_Keberangkatan`) AS `Bulan`, count(month(`pemesananpergi`.`Tanggal_Keberangkatan`)) AS `Crowded_Schedule` FROM `pemesananpergi` GROUP BY month(`pemesananpergi`.`Tanggal_Keberangkatan`) ORDER BY count(month(`pemesananpergi`.`Tanggal_Keberangkatan`)) AS `DESCdesc` ASC  ;

-- --------------------------------------------------------

--
-- Structure for view `profitbulan`
--
DROP TABLE IF EXISTS `profitbulan`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `profitbulan`  AS SELECT month(`pemesananpergi`.`Tanggal_Keberangkatan`) AS `Bulan`, sum(`HitungProfit`(`hargatiket`.`HargaTiket`)) AS `ProfitBulan` FROM ((`pemesananpergi` join `hargatiket`) join `pembayaran`) WHERE `pemesananpergi`.`Nomor_Transaksi_Pergi` = `pembayaran`.`Nomor_Pergi` AND `pembayaran`.`hargaId` = `hargatiket`.`Idharga` GROUP BY month(`pemesananpergi`.`Tanggal_Keberangkatan`)  ;
...
--
-- Indexes for table `hargatiket`
--
ALTER TABLE `hargatiket`
  ADD PRIMARY KEY (`Idharga`),
  ADD KEY `MaskapaiId` (`MaskapaiId`);
...
ALTER TABLE `kuliner`
  ...
  ADD KEY `Nomor_Pergi_Kuliner` (`Nomor_Pergi_Kuliner`);

--
-- Indexes for table `maskapai`
--
ALTER TABLE `maskapai`
  ADD PRIMARY KEY (`MaskapaiId`),
...
ALTER TABLE `pembatalan`
  ADD PRIMARY KEY (`PembatalanID`),
  ADD KEY `Customer_ID` (`Customer_ID`),
  ADD KEY `Nomor_Pergi` (`Nomor_Pergi`),
  ADD KEY `Nomor_Pulang` (`Nomor_Pulang`),
  ADD KEY `Nomor_Bayar` (`Nomor_Bayar`);
...
ALTER TABLE `pembayaran`
  ...
  ADD KEY `Nomor_Pergi` (`Nomor_Pergi`),
  ADD KEY `Nomor_Pulang` (`Nomor_Pulang`),
  ADD KEY `hargaId` (`hargaId`);

--
-- Indexes for table `pemesananpergi`
--
ALTER TABLE `pemesananpergi`
  ADD PRIMARY KEY (`Nomor_Transaksi_Pergi`),
  ADD KEY `Customer_ID` (`Customer_ID`),
  ADD KEY `Bandara_ID` (`Bandara_ID`),
  ADD KEY `Maskapai_ID` (`Maskapai_ID`);

--
-- Indexes for table `pemesananpulang`
--
ALTER TABLE `pemesananpulang`
  ADD PRIMARY KEY (`Nomor_Transaksi_Pulang`),
  ADD KEY `Nomor_Pergi` (`Nomor_Pergi`),
  ADD KEY `Bandara_ID` (`Bandara_ID`),
  ADD KEY `Maskapai_ID` (`Maskapai_ID`);
...
--
-- Indexes for table `tipe_pesawat`
--
ALTER TABLE `tipe_pesawat`
...
--
ALTER TABLE `wisata`
  ADD CONSTRAINT `wisata_ibfk_1` FOREIGN KEY (`Nomor_Pergi`) REFERENCES `pemesananpergi` (`Nomor_Transaksi_Pergi`);
COMMIT;
